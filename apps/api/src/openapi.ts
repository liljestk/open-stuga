...
const stugbyIdParameter = { name: "stugbyId", in: "path", required: true, schema: { type: "string", format: "uuid" } } as const;
const stugbyNodeSecurity = [{ stugbyNodeSignature: [] }] as const;
const stugbyObject = { type: "object", additionalProperties: true } as const;
const stugbyPaths = {
  "/stugbys": {
    get: { tags: ["Stugby"], operationId: "listStugbys", responses: { "200": jsonResponse("Local node identity, server-owned coordinator origin, and joined Stugbys.", stugbyObject) } },
    post: { tags: ["Stugby"], operationId: "createStugby", description: "Creates a coordinator-led federation at the server-owned STUGBY_PUBLIC_ORIGIN. Joining or creating shares no Home data.", requestBody: jsonRequestBody({ type: "object", additionalProperties: false, required: ["name"], properties: { name: { type: "string", maxLength: 200 }, description: { type: ["string", "null"] } } }), responses: { "201": jsonResponse("Created Stugby.", stugbyObject) } },
  },
  "/stugbys/join": { post: { tags: ["Stugby"], operationId: "joinStugby", description: "Consumes a one-time node invitation. No dataset is shared by joining.", requestBody: jsonRequestBody({ type: "object", additionalProperties: false, required: ["coordinatorUrl", "invitationId", "joinSecret"], properties: { coordinatorUrl: { type: "string", format: "uri" }, invitationId: { type: "string" }, joinSecret: { type: "string", writeOnly: true } } }), responses: { "201": jsonResponse("Joined Stugby.", stugbyObject) } } },
  "/stugbys/{stugbyId}": { parameters: [stugbyIdParameter], get: { tags: ["Stugby"], operationId: "getStugby", responses: { "200": jsonResponse("Roster, grants, common property and read-only remote projections.", stugbyObject) } } },
  "/stugbys/{stugbyId}/invitations": { parameters: [stugbyIdParameter], post: { tags: ["Stugby"], operationId: "createStugbyInvitation", requestBody: jsonRequestBody({ type: "object", required: ["role"], properties: { role: { enum: ["property-manager", "participant", "viewer"] }, expiresAt: { type: "string", format: "date-time" } } }), responses: { "201": jsonResponse("One-time invitation; its join secret is returned only once.", stugbyObject) } } },
  "/stugbys/{stugbyId}/invitations/{invitationId}": { parameters: [stugbyIdParameter, { name: "invitationId", in: "path", required: true, schema: { type: "string", format: "uuid" } }], delete: { tags: ["Stugby"], operationId: "revokeStugbyInvitation", responses: { "204": { description: "Invitation revoked" } } } },
  "/stugbys/{stugbyId}/members/{nodeId}": { parameters: [stugbyIdParameter, { name: "nodeId", in: "path", required: true, schema: { type: "string", format: "uuid" } }], patch: { tags: ["Stugby"], operationId: "updateStugbyMember", description: "Deactivation fails while grants or replica-deletion acknowledgements remain outstanding.", requestBody: jsonRequestBody({ type: "object", required: ["role", "state"], properties: { role: { enum: ["steward", "property-manager", "participant", "viewer"] }, state: { enum: ["active", "suspended", "left", "revoked"] } } }), responses: { "200": jsonResponse("Updated node membership.", stugbyObject), "409": { description: "Shared data or deletion acknowledgement is still outstanding" } } } },
  "/stugbys/{stugbyId}/property": { parameters: [stugbyIdParameter], get: { tags: ["Stugby"], operationId: "getStugbySharedProperty", responses: { "200": jsonResponse("Coordinator-owned common-property aggregate.", stugbyObject) } }, put: { tags: ["Stugby"], operationId: "updateStugbySharedProperty", description: "Optimistic update of areas, equipment, notes and maintenance. Integration and control fields are rejected.", requestBody: jsonRequestBody({ type: "object", required: ["baseRevision", "property"], properties: { baseRevision: { type: "integer", minimum: 1 }, property: stugbyObject } }), responses: { "200": jsonResponse("Updated common property.", stugbyObject), "409": { description: "Revision conflict" } } } },
  "/stugbys/{stugbyId}/grants": { parameters: [stugbyIdParameter], post: { tags: ["Stugby"], operationId: "createStugbyShareGrant", description: "Creates an explicit per-Home, per-dataset, per-audience grant with cache, retention, local-ID and telemetry limits.", requestBody: jsonRequestBody(stugbyObject), responses: { "201": jsonResponse("Created share grant.", stugbyObject) } } },
  "/stugbys/{stugbyId}/publications/{houseId}": { parameters: [stugbyIdParameter, { name: "houseId", in: "path", required: true, schema: { type: "string" } }], get: { tags: ["Stugby"], operationId: "getLocalStugbyPublicationCatalog", description: "Local-admin-only mapping used to select a Home's sensors by federation publication ID. It is never sent as a federation dataset.", responses: { "200": jsonResponse("Local publication catalog.", stugbyObject) } } },
  "/stugbys/{stugbyId}/grants/{grantId}": { parameters: [stugbyIdParameter, { name: "grantId", in: "path", required: true, schema: { type: "string", format: "uuid" } }], put: { tags: ["Stugby"], operationId: "updateStugbyShareGrant", description: "Tombstones the previous epoch before publishing the replacement.", requestBody: jsonRequestBody(stugbyObject), responses: { "200": jsonResponse("Updated share grant.", stugbyObject) } }, delete: { tags: ["Stugby"], operationId: "revokeStugbyShareGrant", responses: { "200": jsonResponse("Revoked grant and queued replica purge.", stugbyObject) } } },
  "/stugbys/{stugbyId}/grants/{grantId}/republish": { parameters: [stugbyIdParameter, { name: "grantId", in: "path", required: true, schema: { type: "string", format: "uuid" } }], post: { tags: ["Stugby"], operationId: "republishStugbyGrant", responses: { "202": jsonResponse("Fresh snapshots queued.", { type: "object", required: ["queued"], properties: { queued: { const: true } } }) } } },
  "/stugbys/{stugbyId}/sync": { parameters: [stugbyIdParameter], post: { tags: ["Stugby"], operationId: "syncStugby", responses: { "200": jsonResponse("Outbox flushed and durable coordinator cursor pulled.", stugbyObject) } } },
  "/stugbys/{stugbyId}/telemetry": { parameters: [stugbyIdParameter], get: { tags: ["Stugby"], operationId: "queryStugbyRemoteTelemetry", description: "Queries the separate read-only remote telemetry projection.", responses: { "200": jsonResponse("Granted remote samples.", { type: "object", required: ["samples"], properties: { samples: { type: "array", items: stugbyObject } } }) } } },
  "/stugbys/{stugbyId}/assets/{digest}": { parameters: [stugbyIdParameter, { name: "digest", in: "path", required: true, schema: { type: "string", pattern: "^[a-f0-9]{64}$" } }], get: { tags: ["Stugby"], operationId: "downloadLocalStugbyAsset", description: "Returns a locally cached, access-controlled floor-plan image referenced by a granted structure projection.", responses: { "200": { description: "Granted floor-plan image", content: { "image/png": { schema: { type: "string", format: "binary" } }, "image/jpeg": { schema: { type: "string", format: "binary" } }, "image/webp": { schema: { type: "string", format: "binary" } } } }, "404": { description: "Asset or Stugby not found" } } } },
  "/stugby-protocol/join": { post: { security: [], tags: ["Stugby"], operationId: "acceptStugbyNodeJoin", description: "One-time Ed25519-proven node admission.", requestBody: { required: true, content: { "application/vnd.stugby.v1+json": { schema: stugbyObject } } }, responses: { "201": jsonResponse("Activated node membership.", stugbyObject) } } },
  "/stugby-protocol/stugbys/{stugbyId}/events": { parameters: [stugbyIdParameter], get: { security: stugbyNodeSecurity, tags: ["Stugby"], operationId: "pullStugbyEvents", description: "Cursor-resumable durable event page.", responses: { "200": jsonResponse("Signed event page.", stugbyObject) } }, post: { security: stugbyNodeSecurity, tags: ["Stugby"], operationId: "publishStugbyEvents", description: "At-least-once signed event batch. Only approved read-only datasets and grants are accepted from participants.", requestBody: { required: true, content: { "application/vnd.stugby.v1+json": { schema: stugbyObject } } }, responses: { "200": jsonResponse("Accepted and duplicate event ids.", stugbyObject) } } },
  "/stugby-protocol/stugbys/{stugbyId}/events/stream": { parameters: [stugbyIdParameter], get: { security: stugbyNodeSecurity, tags: ["Stugby"], operationId: "streamStugbyAvailability", description: "SSE carries only durable cursor-availability notifications; event bodies are pulled over HTTPS.", responses: { "200": { description: "Server-Sent Events", content: { "text/event-stream": { schema: { type: "string" } } } } } } },
  "/stugby-protocol/stugbys/{stugbyId}/blobs/{digest}": { parameters: [stugbyIdParameter, { name: "digest", in: "path", required: true, schema: { type: "string", pattern: "^[a-f0-9]{64}$" } }], get: { security: stugbyNodeSecurity, tags: ["Stugby"], operationId: "downloadStugbyBlob", responses: { "200": { description: "Audience-authorized content-addressed floor-plan image", content: { "image/png": { schema: { type: "string", format: "binary" } }, "image/jpeg": { schema: { type: "string", format: "binary" } }, "image/webp": { schema: { type: "string", format: "binary" } } } } } }, put: { security: stugbyNodeSecurity, tags: ["Stugby"], operationId: "uploadStugbyBlob", responses: { "201": jsonResponse("Validated content-addressed image.", stugbyObject) } } },
  "/stugby-protocol/stugbys/{stugbyId}/property": { parameters: [stugbyIdParameter], put: { security: stugbyNodeSecurity, tags: ["Stugby"], operationId: "forwardStugbyPropertyUpdate", requestBody: { required: true, content: { "application/vnd.stugby.v1+json": { schema: stugbyObject } } }, responses: { "200": jsonResponse("Updated coordinator-owned property.", stugbyObject) } } },
  "/stugby-protocol/stugbys/{stugbyId}/deletion-receipts": { parameters: [stugbyIdParameter], post: { security: stugbyNodeSecurity, tags: ["Stugby"], operationId: "acknowledgeStugbyDeletion", responses: { "204": { description: "Replica purge acknowledged" } } } },
} as const;
...
const combinedOpenApiDocument = {
  ...
  tags: [
    ...
    { name: "Context" }, { name: "Security" }, { name: "Weather" }, { name: "Physics" }, { name: "Integrations" }, { name: "Testing" }, { name: "Measurements" }, { name: "Stugby", description: "Coordinator-led federation with explicit read-only dataset grants. Credentials, integrations, account identities, and control commands are outside this protocol." },
  ...
  components: {
    securitySchemes: {
      ...
      stugbyNodeSignature: { type: "apiKey", in: "header", name: "X-Stugby-Signature", description: "Ed25519 request signature. X-Stugby-Node-Id, X-Stugby-Timestamp, X-Stugby-Request-Id, and X-Stugby-Protocol are also mandatory and replay-checked." },
    },
    parameters: {
