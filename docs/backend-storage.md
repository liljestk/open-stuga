...
Credential bytes are the deliberate exception. Home Assistant tokens, TP-Link and Telegram credentials, complete webhook URLs/bearers, and Apple Notes grant-token hashes remain in the owner-protected integration secrets file; putting credentials or capability-bearing URL query strings in the broad core database would weaken recovery and access boundaries. SQLite stores typed non-secret references, Home ownership, sanitized Home Assistant endpoints, TP-Link hosts, safe display labels, active/retired lifecycle state, and an append-only revision snapshot for each change. Existing version-1 secret files are reconciled on startup without rewriting live credentials. A validated file can soft-retire references that are no longer present; a missing, unreadable, or explicitly partial file never prunes last-known metadata. Records are not hard-deleted, including after a Home is removed.

Environment variables remain valid deployment overrides. An environment webhook takes precedence and is mirrored atomically into the protected secrets file so a complete backup can restore it after the override is removed; SQLite receives only the safe singleton reference and revision. If protected-file mirroring is unavailable, the environment configuration continues to operate but the backup must retain the deployment secret separately. In-process caches, connection status, discovery results, SSE listeners, in-flight work, and replay position are reconstructible runtime state; browser storage contains presentation preferences, not authoritative Property data.
